...
public class PedidoController {
    ...
    public String getPort(@Value("${local.server.port}") String porta){
        ...
        return String.format("Request da instânica recebida na port %s ", porta);
    ...
    @PutMapping("/{id}/pago")
    public ResponseEntity<String> aprovarPagamanetoDoPedido(@PathVariable
                                                            @NotNull Long id){

        service.aprovarPagamentoDoPedido(id);
        String msg = "Pedido pago, aguardando confirmação";
